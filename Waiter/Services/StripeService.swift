...
class StripeService {
    ...
    // CREATE STRIPE CUSTOMER createStripeCustomer
    ...
    func createStripeConnectAccount(host: Host, firebaseUID: String, _ completion: @escaping (Bool, HTTPSCallableResult?) -> ()) {
        let params: [String: Any] = [
            "email": host.email,
            "first_name": host.fn,
            "last_name": host.ln,
            "ssn_last_4": host.last4ssn,
            "phone": host.phone,
            "city": host.address.city,
            "address_line_1": host.address.line1,
            "zip": host.address.zip,
            "state": host.address.state,
            "firebaseUID": firebaseUID,
            "birth_day": host.birthday.day,
            "birth_month": host.birthday.month,
            "birth_year": host.birthday.year
        ]
        functions.httpsCallable("createStripeAccount").call(params) { (result, error) in
            if let error = error {
                print(error.localizedDescription)
                completion(false, nil)
            } else if let result = result {
    ...
    // MARK: - NON FIREBASE CLOUD FUNCTION ENDPOINTS
    
      func createConnectAccount(currentUser: Person,
                                  source: String,
                                  completion: @escaping STPJSONResponseCompletionBlock) {
            let e = currentUser.email ?? DataService.instance.retrieveAuthUsername()
            let fName = currentUser.fName ?? DataService.instance.retrieveFirstName()
            let lName = currentUser.lName ?? DataService.instance.retrieveLastName()
            var params: [String: Any]!
            var url: URL!
            if currentUser.businessType == "individual" {
                url = baseURL.appendingPathComponent("v1/setup-individual-connect-account")
                print("------------ INDIVIDUAL CONNECT ACCOUNT ----- PARAMS BEING PASSED IN -----------")
                print(source)
                params = [
                    "email": e!,
                    "source": source,
                    ...
                    "phone_number": "+1"+currentUser.phoneNumber,
                    "birthday_month": currentUser.monthBirthday!,
                    "birthday_day": currentUser.dayBirthday!,
                    "birthday_year": currentUser.yearBirthday!,
                    "ssn_3": currentUser.ssn3!,
                    "address_line_1": currentUser.addressLine1!,
                    "address_city": currentUser.addressCity!,
                    "address_state": currentUser.addressState!,
                    "address_zip_code": currentUser.addressZipCode!
                ]

            } else { // BUSINESS TYPE == COMPANY
                guard let c = currentUser.company else { return }
